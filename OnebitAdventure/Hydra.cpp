...
void Hydra::Update()
{
	...
	if (moveTimer->Elapsed(0.3f) && Level1::player->IsMoving()) {
		if (rand() % 100 < 50 && contadorMovimento == 0 && !isAttacked) {
			HandleMovement(100.0f);
		}
		else if (rand() % 100 < 0 && contadorMovimento == 0 && !isAttacked) {
			Ataque2();
		}
		else if (rand() % 100 < 90 && contadorMovimento == 0 && !isAttacked) {
...
void Hydra::OnCollision(Object* obj)
{
	...
	if (type == PLAYER)
	{
		...
		if (life <= 0)
		{
			...
			Level1::scene->Delete(this, MOVING);					// deleta esse menor
			((Character*)player)->SetXp(200 * (level));	            // Adiciona a experiência ao player
		}

		isHit = false;
	}
	else if (type != ENEMY && type != BOSSATACK)
	{
